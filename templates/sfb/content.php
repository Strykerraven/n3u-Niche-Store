	...
	*/
	...
	switch ($n3u_inputVars['x']){
		...
		case "login": // If login, do this
			...
			if((isset($n3u_configVars['reCaptcha_pubKey']) && $n3u_configVars['reCaptcha_pubKey'] != '') && (isset($n3u_configVars['reCaptcha_privKey']) && $n3u_configVars['reCaptcha_privKey'] != '')) {
		...
		case "page":
			...
			if (isset($n3u_inputVars['page']) && $n3u_inputVars['page'] != NULL){
				...
				if (file_exists($n3u_configVars['include_dir'] . 'custom/'.$url['host'] . '_' . $n3u_inputVars['page'].'.php')){
					...
					n3u_Error(404,'Page <em>'. $n3u_configVars['include_dir'] . 'custom/' . $n3u_inputVars['page'] . '</em> doesn\'t exist.');
				}
				unset($url);
