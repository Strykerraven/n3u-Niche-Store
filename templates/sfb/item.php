	...
	**/
	...
	if(!isset($n3u_results) || $prosperentApi->gettotalRecordsFound() == NULL){
		...
		foreach($n3u_results as $n3u_result){
			...
			if(($n3u_result['upc']) && (strlen('0'.$n3u_result['upc']) == '13')){echo "\t\t\t\t\t\t" . '&nbsp;<h5>'.$n3u_lang['UPC'].'</h5>&nbsp;<span class="UPC_Info" itemprop="gtin13">0' . $n3u_result['upc'] . '</span><br />' . PHP_EOL;}
			if($n3u_result['isbn']){echo "\t\t\t\t\t\t" . '&nbsp;<h5>'.$n3u_lang['ISBN'].'</h5>&nbsp;<span class="isbn">' . $n3u_result['isbn'] . '</span><br />' . PHP_EOL;}
