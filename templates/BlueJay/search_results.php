		...
		foreach($n3u_results as $n3u_result){
			...
			if(($n3u_result['keyword'] || $n3u_result['keywords']) && $n3u_result['affiliate_url'] && $n3u_result['image_url']){
				...
				if($n3u_configVars['CleanUrls'] == TRUE){
					echo "\t\t\t\t\t\t" . '<a class="link" href="item_' . urlencode($n3u_result['catalogId']) . '.htm" id="' . n3u_IdCleaner($n3u_result['catalogId']) . '" target="_self" title="' . n3u_TitleCleaner($n3u_result['keyword']) . '">' . PHP_EOL
					. "\t\t\t\t\t\t\t" . '<h4 itemprop="name">' . $n3u_result['keyword'] . '</h4>' . PHP_EOL;
					if($n3u_configVars['cacheImgs'] == TRUE){
						echo "\t\t\t\t\t\t\t" . '<img alt="' . n3u_TitleCleaner($n3u_result['keyword']) . '" class="image" height="' . $n3u_img_size['1'] . '" id="' . $n3u_result['catalogId'] . '-image" src="' . @n3u_CacheImage($n3u_result['image_url'],$n3u_result['catalogId']) . '" width="' . $n3u_img_size['0'] . '" />' . PHP_EOL;
					}else{
						echo "\t\t\t\t\t\t\t" . '<img alt="' . n3u_TitleCleaner($n3u_result['keyword']) . '" class="image" height="' . $n3u_img_size['1'] . '" id="' . $n3u_result['catalogId'] . '-image" src="' . $n3u_result['image_url'] . '" width="' . $n3u_img_size['0'] . '" />' . PHP_EOL;
					}
					echo "\t\t\t\t\t\t" . '</a>' . PHP_EOL;
				}else{
					echo "\t\t\t\t\t\t" . '<a class="link" href="' . $n3u_configVars['self'] . '?x=item&amp;item=' . urlencode($n3u_result['catalogId']) . '" id="' . n3u_IdCleaner($n3u_result['catalogId']) . '" target="_self" title="' . n3u_TitleCleaner($n3u_result['keyword']) . '">' . PHP_EOL
					. "\t\t\t\t\t\t\t" . '<h4 itemprop="name">' . $n3u_result['keyword'] . '</h4>' . PHP_EOL;
					if($n3u_configVars['cacheImgs'] == TRUE){
						echo "\t\t\t\t\t\t\t" . '<img alt="' . n3u_TitleCleaner($n3u_result['keyword']) . '" class="image" height="' . $n3u_img_size['1'] . '" id="' . $n3u_result['catalogId'] . '-image" src="' . @n3u_CacheImage($n3u_result['image_url'],$n3u_result['catalogId']) . '" width="' . $n3u_img_size['0'] . '" />' . PHP_EOL;
					}else{
