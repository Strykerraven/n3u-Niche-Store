...
//	preprint_r($n3u_merchants);
	...
	. "\t\t\t\t\t\t" . '<label>' . $n3u_lang['BrowseBySearch'] . '</label>' . PHP_EOL
	...
	if(isset($n3u_inputVars['querySuggestion']) && ($n3u_inputVars['querySuggestion'] != NULL)){
	...
	. "\t\t\t\t\t" . '<div id="Brands" itemscope itemtype="http://schema.org/Brand">' . PHP_EOL
	. "\t\t\t\t\t\t" . '<label>'.$n3u_lang['BrowseByBrand'].'</label>' . PHP_EOL;
	...
	foreach($prosperentApi->getData('') as $row){
		if($n3u_configVars['CleanUrls'] == TRUE){
			...
			if($n3u_configVars['cacheImgs'] == TRUE){
				$brand_img[] = "\t\t\t\t\t\t" . '<a class="link" href="' . $n3u_brand_url . '" itemprop="url" title="' . n3u_TitleCleaner($row['brand']) . '"><img alt="' . n3u_TitleCleaner($row['brand']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="logo" src="' . n3u_CacheImage(filter_var($row['logoUrl'], FILTER_SANITIZE_URL),n3u_IdCleaner($row['brand'])) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL;
			}else{
				$brand_img[] = "\t\t\t\t\t\t" . '<a class="link" href="' . $n3u_brand_url . '" itemprop="url" title="' . n3u_TitleCleaner($row['brand']) . '"><img alt="' . n3u_TitleCleaner($row['brand']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="logo" src="' . filter_var($row['logoUrl'], FILTER_SANITIZE_URL) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL;
			}
		}else{
			if($n3u_configVars['cacheImgs'] == TRUE){
				$brand_img[] = "\t\t\t\t\t\t" . '<a class="link" href="index.php?x=search&amp;q=&amp;b=' . strtolower(n3u_UrlEncode(str_replace(array('\'',',','.','/','  '),array('','','','',' '),$row['brand']))) . '&amp;m=&amp;sort=' . @$n3u_inputVars['sort'] . '&amp;p=1" itemprop="url" title="' . n3u_TitleCleaner($row['brand']) . '"><img alt="' . n3u_TitleCleaner($row['brand']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="logo" src="' . @n3u_CacheImage(filter_var($row['logoUrl'], FILTER_SANITIZE_URL),n3u_IdCleaner($row['brand'])) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL;
			}else{
				$brand_img[] = "\t\t\t\t\t\t" . '<a class="link" href="index.php?x=search&amp;q=&amp;b=' . strtolower(n3u_UrlEncode(str_replace(array('\'',',','.','/','  '),array('','','','',' '),$row['brand']))) . '&amp;m=&amp;sort=' . @$n3u_inputVars['sort'] . '&amp;p=1" itemprop="url" title="' . n3u_TitleCleaner($row['brand']) . '"><img alt="' . n3u_TitleCleaner($row['brand']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="logo" src="' . filter_var($row['logoUrl'], FILTER_SANITIZE_URL) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL;
			}
		}
	}
	sort($brand_img);
	// preprint_r($brand_img);
	$i=0;
	...
	. "\t\t\t\t\t" . '<div id="Merchants" itemscope itemtype="http://schema.org/Offer">' . PHP_EOL
	. "\t\t\t\t\t\t" . '<label itemprop="name" >'.$n3u_lang['BrowseByMerchant'].'</label>' . PHP_EOL;
	...
	foreach($prosperentApi->getData('') as $row){	
		if($n3u_configVars['CleanUrls'] == TRUE){
			...
			if($n3u_configVars['cacheImgs'] == TRUE){
				$merchant_img[] = "\t\t\t\t\t\t" . '<a class="link" href="' . $n3u_merchant_url . '" itemprop="url" title="' . n3u_TitleCleaner($row['merchant']) . '"><img alt="' . n3u_TitleCleaner($row['merchant']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="image" src="' . n3u_CacheImage(filter_var($row['logoUrl'], FILTER_SANITIZE_URL),n3u_IdCleaner($row['merchant'])) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL;
			}else{
				$merchant_img[] = "\t\t\t\t\t\t" . '<a class="link" href="' . $n3u_merchant_url . '" itemprop="url" title="' . n3u_TitleCleaner($row['merchant']) . '"><img alt="' . n3u_TitleCleaner($row['merchant']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="image" src="' . filter_var($row['logoUrl'], FILTER_SANITIZE_URL) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL;
			}
		}else{
			if($n3u_configVars['cacheImgs'] == TRUE){
				$merchant_img[] = "\t\t\t\t\t\t" . '<a href="index.php?x=search&amp;q=&amp;b=&amp;m=' . strtolower(n3u_UrlEncode(str_replace(array('\'',',','.','/','  '),array('','','','',' '),$row['merchant']))) . '&amp;sort=' . @$n3u_inputVars['sort'] . '&amp;p=1" itemprop="url" title="' . n3u_TitleCleaner($row['merchant']) . '"><img alt="' . n3u_TitleCleaner($row['merchant']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="image" src="' . n3u_CacheImage(filter_var($row['logoUrl'], FILTER_SANITIZE_URL),n3u_IdCleaner($row['merchant'])) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL; // index.php?x=&q=&b=&m=&sort=&p=
			}else{
				$merchant_img[] = "\t\t\t\t\t\t" . '<a href="index.php?x=search&amp;q=&amp;b=&amp;m=' . strtolower(n3u_UrlEncode(str_replace(array('\'',',','.','/','  '),array('','','','',' '),$row['merchant']))) . '&amp;sort=' . @$n3u_inputVars['sort'] . '&amp;p=1" itemprop="url" title="' . n3u_TitleCleaner($row['merchant']) . '"><img alt="' . n3u_TitleCleaner($row['merchant']) . '" class="image" height="' . $n3u_img_size['1'] . '" itemprop="image" src="' . filter_var($row['logoUrl'], FILTER_SANITIZE_URL) . '" width="' . $n3u_img_size['0'] . '" /></a>' . PHP_EOL; // index.php?x=&q=&b=&m=&sort=&p=
			}
		}
	}
	sort($merchant_img);
	...
	. "\t\t\t\t\t" . '<p class="greeting" id="Greeting">'.$n3u_lang['Greeting'].'</p>' . PHP_EOL
	. "\t\t\t\t\t" . '<hr class="hr" />' . PHP_EOL
	. "\t\t\t\t\t" . '<div id="CategoriesIndex">' . PHP_EOL
	. "\t\t\t\t\t\t" . '<label>'.$n3u_lang['BrowseByCategory'].'</label>' . PHP_EOL;
	...
	. "\t\t\t\t\t\t" . '<label>'.$n3u_lang['BrowseByOther'].'</label>' . PHP_EOL;
	...
	if(defined('admin')){
		...
		. "\t\t\t\t\t\t" . '<label>'.$n3u_lang['BrowseByAdmin'].'</label>' . PHP_EOL
		. "\t\t\t\t\t\t" . '<ul>' . PHP_EOL;
