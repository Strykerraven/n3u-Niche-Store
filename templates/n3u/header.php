<?php 
	...
	**/
	...
	//	. "\t\t\t\t\t\t\t\t" . '<li><a href="./" id="MenuHome" target="_self" title="' . $n3u_lang['Home'] . '" type="text/html">' . $n3u_lang['Home'] . '</a></li>' . PHP_EOL
		. "\t\t\t\t\t\t\t\t" . '<li>' . PHP_EOL
		. "\t\t\t\t\t\t\t\t\t" . '<a href="./" target="_self" tabindex="1" title="' . n3u_TitleCleaner($n3u_configVars['SiteName']) . '">' . $n3u_configVars['SiteName'] . '</a>' . PHP_EOL
		. "\t\t\t\t\t\t\t\t\t" . '<ul>' . PHP_EOL;
		...
		. "\t\t\t\t\t\t\t\t" . '<li>' . PHP_EOL;
		if($n3u_configVars['CleanUrls'] == TRUE){
			echo "\t\t\t\t\t\t\t\t\t" . '<a href="'.filter_var('search___'. urlencode($n3u_configVars['defaultKeyword']) . '-REL-1.htm', FILTER_SANITIZE_URL).'"  tabindex="2" title="Categories">' . $n3u_lang['Categories'] . '</a>' . PHP_EOL;
		}else{
			echo "\t\t\t\t\t\t\t\t\t" . '<a href="' . filter_var($n3u_configVars['self'] . '?x=search&amp;m=&amp;b=&amp;q='. urlencode($n3u_configVars['defaultKeyword']), FILTER_SANITIZE_URL) . '" tabindex="2" title="Categories">' . $n3u_lang['Categories'] . '</a>' . PHP_EOL;
		}
...
//		. "\t\t\t\t\t\t\t\t" . '<li><a title="Browse by...">Browse by...</a></li>' . PHP_EOL // Categories
		...
		if(defined('admin')){
			...
			if($n3u_configVars['CleanUrls'] == TRUE){
				...
				echo "\t\t\t\t\t\t\t\t\t" . '<a href="index.php?x=admin" tabindex="3" title="' . n3u_TitleCleaner($n3u_lang['Administration']) . '">' . $n3u_lang['Administration'] . '</a>' . PHP_EOL;
			}
			echo "\t\t\t\t\t\t\t\t\t" . '<ul>' . PHP_EOL;
			...
			if($n3u_configVars['CleanUrls'] == TRUE){
				foreach($n3u_configVars['AdminCatList'] as $n3u_Category){
					...
					echo "\t\t\t\t\t\t\t\t\t\t" . '<li><a class="link" href="' . $n3u_category_url . '" tabindex="3" title="' . n3u_TitleCleaner($n3u_Category). '">' . ucfirst($n3u_Category) . '</a></li>' . PHP_EOL;
				}
			}else{
				foreach($n3u_configVars['AdminCatList'] as $n3u_Category){
			...
			echo "\t\t\t\t\t\t\t\t\t\t" . '<li><a class="link" href="'. $n3u_configVars['self'] .'?x=logout" title="' . n3u_TitleCleaner($n3u_lang['LogOut']) . '">' . $n3u_lang['LogOut'] . '</a></li>' . PHP_EOL
			. "\t\t\t\t\t\t\t\t\t\t" . '<li>' . PHP_EOL
	...
	switch($n3u_inputVars['x']){
		...
		case "admin": // If Admin, do this
			...
			. "\t\t" . '<meta name="author" content="n3u.com">' . PHP_EOL
		...
		case "index": // If index, do this
			...
			. "\t\t" . '<meta name="application-name" content="n3u Niche Store ' . $n3u_configVars['Version']. '">' . PHP_EOL
			. "\t\t" . '<meta name="author" content="n3u.com">' . PHP_EOL
			. "\t\t" . '<meta name="description" content="Thanks for visiting ' . $n3u_configVars['SiteName'] . ', Your one stop for anything relating to ' . $n3u_configVars['defaultKeyword'] . '. Please enjoy the website!">' . PHP_EOL;
			if(!isset($n3u_results) || @$prosperentApi->gettotalRecordsFound() == NULL){
