...
//
	...
	function n3u_WriteBlockConfig($domainConfig=NULL){
		...
		$configFile .= "\t" . '); // n3u Niche Store is brought to you by https://github.com/Strykerraven' . PHP_EOL
				. '?>';
	...
	function n3u_CacheCSS(){
		...
		if(defined('admin') || $n3u_configVars['caching'] == FALSE){ // if admin or if caching is disabled
			...
			if(file_exists($n3u_cacheFilePath) && time() - $n3u_configVars['lifetime'] < filemtime($n3u_cacheFilePath)){ // else if a recent cache file exist
				...
				foreach($css as $file){
					require_once($file);
				} // require every file from the css array
	...
	function n3u_CacheImage($src_img, $val){
		global $n3u_configVars;
		//	global $n3u_inputVars;
		if($n3u_configVars['cacheImgs'] == FALSE){
			return $src_img;
	...
	function n3u_CacheJS(){
		...
		if(defined('admin') || $n3u_configVars['caching'] == FALSE){ // if admin or if caching is disabled
			...
			foreach(glob($n3u_configVars['Template_Dir'] . $n3u_configVars['Template_Name'] . '/*.js') as $filename){
				echo "\t\t" . '<script defer src="' . $filename . '"></script>' . PHP_EOL;
			} // get template js files
			foreach(glob($n3u_configVars['blocks_dir'] . '*/*.js') as $filename){
				if(preg_match('/disabled/i', $filename) != TRUE){
					echo "\t\t" . '<script defer src="' . $filename . '"></script>' . PHP_EOL;
				}
			} // get block js files
			if($n3u_configVars['jScroll'] == TRUE){
				echo "\t\t" . '<script defer src="' . $n3u_configVars['include_dir'] . 'jquery.jscroll.min.js"></script>' . PHP_EOL;
			}
		}else{ // if not admin and caching is enabled and cache is old
			$url=parse_url($n3u_ServerVars['HTTP_HOST']);
			if(!isset($url['host']) || $url['host'] == NULL){
			...
			if(file_exists($n3u_cacheFilePath) && time() - $n3u_configVars['lifetime'] < filemtime($n3u_cacheFilePath)){ // else if a recent cache file exist
				...
				foreach(glob($n3u_configVars['blocks_dir'] . '*/*.js') as $filename){ // get block js files
					if(preg_match('/disabled/i', $filename) != TRUE){
						$js[]=$filename;
					} // add each filename into js array but ignore disabled blocks
				}
				ob_start();
				foreach($js as $file){
					require_once($file);
				} // require every file from the js array
			...
			if($n3u_configVars['jScroll'] == TRUE){
				echo "\t\t" . '<script defer src="' . $n3u_configVars['include_dir'] . 'jquery.jscroll.min.js" type="text/javascript"></script>' . PHP_EOL;
			}
			unset($n3u_cacheFilePath, $url);
		}
	}
	...
	/**
	  ...
	  by allocating Strykerraven's api key instead of theirs for that specific item
	  For example, if a val of 10 is passed (which is default) that translates
	  into Strykerraven having a 1 out of a 10 chance to have it's API Key applied
	  ...
	  though and Strykerraven forces nothing.
	 */
	function n3u_CheckApiKey($api_key, $val=10){ // Check API Key
		...
		return $api_key; // returns either the sites api key, or Strykerravens if conditions are met
	}
		...
		if($val == null){ // empty, clear all
			...
			if($val2 == TRUE){ // last modified
				foreach($files as $file){
					if(filemtime($file) < strtotime('-' . $val . ' days')){
						@unlink($file);
					} // delete file
				}
			}elseif($val2 == FALSE){ // last accessed
				foreach($files as $file){
					if(fileatime($file) < strtotime('-' . $val . ' days')){
						@unlink($file);
					} // delete file
				}
			}
	...
	function n3u_Config(){ // Check installation, get data
		...
		if(file_exists($url['host'] . '_config.php')){
			require_once($url['host'] . '_config.php');
		} // This should get config for sub domains and overtide main configs array in the process.
	...
	function n3u_Debug($n3u_val, $info=FALSE){
		...
		if($info == TRUE){
			...
			if(isset($n3u_val) && $n3u_val != NULL){
				echo "\t\t\t\t\t" . '<pre>' . PHP_EOL;
				foreach($n3u_val as $n3u_row){
					print_r(str_replace('&', '&amp;', array_combine(array_keys($n3u_row), array_values($n3u_row))));
				}
				echo "\t\t\t\t\t" . '</pre>' . PHP_EOL;
			}
		}
	...
	function n3u_Defaults(){
		...
		if(!isset($n3u_inputVars['b']) || $n3u_inputVars['b'] == array('Any' || 'Unknown')){
			$n3u_inputVars['b']='';
		}
		//	if(!isset($n3u_inputVars['compare'])){$n3u_inputVars['compare'] = NULL;}
		if(!isset($n3u_inputVars['m']) || $n3u_inputVars['m'] == array('Any' || 'Unknown')){
			$n3u_inputVars['m']='';
		}
		if(!isset($n3u_inputVars['url'])){
			unset($n3u_inputVars['url']);
		}
		...
		if(!isset($n3u_inputVars['p']) || ($n3u_inputVars['p'] == NULL)){
			$n3u_inputVars['p']='1';
		}
		if(!isset($n3u_inputVars['sort']) || !in_array($n3u_inputVars['sort'], array('ASC', 'DESC', 'REL'))){
			$n3u_inputVars['sort']='REL';
		}
		if(!isset($n3u_inputVars['item'])){
			$n3u_inputVars['item']='';
		...
		// defaults
		if(!isset($n3u_ServerVars['HTTP_CF_CONNECTING_IP'])){
			$n3u_configVars['visitor_ip']=@$n3u_ServerVars['REMOTE_ADDR'];
		}else{
			$n3u_configVars['visitor_ip']=$n3u_ServerVars['HTTP_CF_CONNECTING_IP'];
		}  // CloudFlare is a reverse proxy, so all ips look like they originate from cloudflare, CloudFlare passes REMOTE_ADDR as HTTP_CF_CONNECTING_IP
		if(!isset($n3u_ServerVars['HTTP_REFERER'])){
			$n3u_configVars['referrer']='';
			$n3u_ServerVars['HTTP_REFERER']='';
		}else{
			$n3u_configVars['referrer']=$n3u_ServerVars['HTTP_REFERER'];
		} // Auto determines Visitors Referrer
		if(preg_match("/" . @$n3u_ServerVars['HTTP_HOST'] . "/i", $n3u_ServerVars['HTTP_REFERER'])){
			$n3u_configVars['referrer']='';
			$n3u_ServerVars['HTTP_REFERER']='';
		} // empties referrer if from site.
		if(!isset($n3u_configVars['api_key']) || $n3u_configVars['api_key'] == NULL){
			$n3u_configVars['api_key']='8703b59ba1a6449fa7f361fa13982524';
		} // Use n3u's if no other is set (to ensure script functions)
		if(!isset($n3u_configVars['blocks_dir']) || $n3u_configVars['blocks_dir'] == NULL){
			$n3u_configVars['blocks_dir']='blocks/';
		}
		if(!isset($n3u_configVars['cacheBackend']) || $n3u_configVars['cacheBackend'] == NULL){
			$n3u_configVars['cacheBackend']='File';
		}
		if(!isset($n3u_configVars['cacheImgs']) || $n3u_configVars['cacheImgs'] == NULL){
			$n3u_configVars['cacheImgs']=FALSE;
		}
		if(!isset($n3u_configVars['cache_dir']) || $n3u_configVars['cache_dir'] == NULL){
			$n3u_configVars['cache_dir']='cache/';
		}
		if(!isset($n3u_configVars['caching']) || $n3u_configVars['caching'] == NULL){
			$n3u_configVars['caching']=FALSE;
		}
		if(!isset($n3u_configVars['ClearCacheFreq']) || $n3u_configVars['ClearCacheFreq'] == NULL){
			$n3u_configVars['ClearCacheFreq']=100;
		}
		if(!isset($n3u_configVars['ClearImgCacheFreq']) || $n3u_configVars['ClearImgCacheFreq'] == NULL){
			$n3u_configVars['ClearImgCacheFreq']=100;
		}
		if(!is_dir($n3u_configVars['cache_dir'])){
			mkdir($n3u_configVars['cache_dir']);
			fclose(fopen($n3u_configVars['cache_dir'] . 'index.php', 'w'));
		} // Auto creates cache folder & index.php
		$url=parse_url($n3u_ServerVars['HTTP_HOST']);
		if(!isset($url['host']) || $url['host'] == NULL){
			if(isset($url['path']) && $url['path'] != NULL){ // Check to see if host is classified as path
				$url['host']=$url['path'];
		...
		if(!is_dir($n3u_configVars['cache_dir'] . $url['host'] . '/')){
			mkdir($n3u_configVars['cache_dir'] . $url['host'] . '/');
			fclose(fopen($n3u_configVars['cache_dir'] . $url['host'] . '/' . 'index.php', 'w'));
		} // Auto creates cache folder & index.php
		unset($url);
		if(!isset($n3u_configVars['Categories']) || $n3u_configVars['Categories'] == NULL){
			$n3u_configVars['Categories']='7.2v,9.6v,12v,14.4v,18v,20v';
		}
		if(!isset($n3u_configVars['CategoryFilters']) || $n3u_configVars['CategoryFilters'] == NULL){
			$n3u_configVars['CategoryFilters']='refurb|refurbished|recondition|reconditioned';
		}
		$n3u_configVars['CategoryFilters']=str_replace(array(' , ', ', ', ',', ' | ', '| ', ' '), array('|', '|', '|', '|', '|', '%20'), $n3u_configVars['CategoryFilters']);
		if(!isset($n3u_configVars['CleanUrls']) || $n3u_configVars['CleanUrls'] == NULL){
			$n3u_configVars['CleanUrls']=FALSE;
		}
		if(!isset($n3u_configVars['commissionDateMonths']) || $n3u_configVars['commissionDateMonths'] == NULL){
			$n3u_configVars['commissionDateMonths']=3;
		}
		if(!isset($n3u_configVars['commissionDateRange']) || $n3u_configVars['commissionDateRange'] == NULL){
			$n3u_configVars['commissionDateRange']=date('Ymd', strtotime('-' . $n3u_configVars['commissionDateMonths'] . ' months -1 day')) . ',' . date('Ymd', strtotime('yesterday'));
		}
		if(!isset($n3u_configVars['debug']) || $n3u_configVars['debug'] == NULL){
			$n3u_configVars['debug']=FALSE;
		}
		if(!isset($n3u_configVars['defaultKeyword']) || $n3u_configVars['defaultKeyword'] == NULL){
			$n3u_configVars['defaultKeyword']='Cordless Drill';
		}
		if(!isset($n3u_inputVars['q']) || $n3u_inputVars['q'] == NULL){
			$n3u_inputVars['q']=$n3u_configVars['defaultKeyword'];
		}
		if($n3u_inputVars['q'] != $n3u_configVars['defaultKeyword']){
			$n3u_inputVars['q']=$n3u_configVars['defaultKeyword'] . ' ' . str_replace($n3u_configVars['defaultKeyword'] . ' ', '', $n3u_inputVars['q']);
		}else{
			$n3u_inputVars['q']=$n3u_configVars['defaultKeyword'];
		}
		if($n3u_inputVars['q'] == $n3u_configVars['defaultKeyword'] . ' ' . $n3u_configVars['defaultKeyword']){
			$n3u_inputVars['q']=$n3u_configVars['defaultKeyword'];
		}
		if(!isset($n3u_configVars['defaultLanguage']) || $n3u_configVars['defaultLanguage'] == NULL){
			$n3u_configVars['defaultLanguage']='en-us';
		}
		if(!isset($n3u_configVars['enableCoupons']) || $n3u_configVars['enableCoupons'] == NULL){
			$n3u_configVars['enableCoupons']=FALSE;
		}
		if(!isset($n3u_configVars['enableFacets']) || $n3u_configVars['enableFacets'] == NULL){
			$n3u_configVars['enableFacets']=TRUE;
		}
		if(!isset($n3u_configVars['enableJsonCompression']) || $n3u_configVars['enableJsonCompression'] == NULL){
			$n3u_configVars['enableJsonCompression']=TRUE;
		}
		if(!isset($n3u_configVars['enableQuerySuggestion']) || $n3u_configVars['enableQuerySuggestion'] == NULL){
			$n3u_configVars['enableQuerySuggestion']=TRUE;
		}
		if(!isset($n3u_inputVars['lang']) || $n3u_inputVars['lang'] == NULL){
			$n3u_inputVars['lang']=$n3u_configVars['defaultLanguage'];
		} // set default language
		if(!isset($n3u_configVars['include_dir']) || $n3u_configVars['include_dir'] == NULL){
			$n3u_configVars['include_dir']='inc/';
		}
		if(!is_dir($n3u_configVars['include_dir'])){
			mkdir($n3u_configVars['include_dir']);
			fclose(fopen($n3u_configVars['include_dir'] . 'index.php', 'w'));
		} // Auto creates inc folder & index.php
		if(!is_dir($n3u_configVars['include_dir'] . 'configs/')){
			mkdir($n3u_configVars['include_dir'] . 'configs/');
			fclose(fopen($n3u_configVars['include_dir'] . 'configs/index.php', 'w'));
		} // Auto creates custom folder & index.php
		if(!is_dir($n3u_configVars['include_dir'] . 'custom/')){
			mkdir($n3u_configVars['include_dir'] . 'custom/');
			fclose(fopen($n3u_configVars['include_dir'] . 'custom/index.php', 'w'));
		} // Auto creates custom folder & index.php
		if(!is_dir($n3u_configVars['include_dir'] . 'messages/')){
			mkdir($n3u_configVars['include_dir'] . 'messages/');
			fclose(fopen($n3u_configVars['include_dir'] . 'messages/index.php', 'w'));
		} // Auto creates custom folder & index.php
		if(!isset($n3u_configVars['img_dir']) || $n3u_configVars['img_dir'] == NULL){
			$n3u_configVars['img_dir']='images/';
		}
		if(!isset($n3u_configVars['img_size']) || $n3u_configVars['img_size'] == NULL){
			$n3u_configVars['img_size']='125x125';
		}
		if(!isset($n3u_configVars['language_dir']) || $n3u_configVars['language_dir'] == NULL){
			$n3u_configVars['language_dir']='languages/';
		}
		if(!isset($n3u_configVars['lifetime']) || $n3u_configVars['lifetime'] == NULL){
			$n3u_configVars['lifetime']='86400';
		}
		if(!isset($n3u_configVars['limit']) || $n3u_configVars['limit'] == NULL){
			$n3u_configVars['limit']='10';
		}
		if(!isset($n3u_configVars['username']) || $n3u_configVars['username'] == NULL){
			$n3u_configVars['username']='n3uadmin';
		}
		if(!isset($n3u_configVars['password']) || $n3u_configVars['password'] == NULL){
			$n3u_configVars['password']='n3upass';
		}
		if(!isset($n3u_configVars['location']) || $n3u_configVars['location'] == NULL){
			$n3u_configVars['location']='http://' . $n3u_ServerVars['HTTP_HOST'] . $n3u_ServerVars['REQUEST_URI'];
		} // Auto determines page location, change to https only if your server supports
		if(!isset($n3u_configVars['Prosperent_Endpoint']) || $n3u_configVars['Prosperent_Endpoint'] == NULL){
			$n3u_configVars['Prosperent_Endpoint']='USA';
		} // Defaults to USA
		if(!isset($n3u_configVars['Prosperent_UserID']) || $n3u_configVars['Prosperent_UserID'] == NULL){
			$n3u_configVars['Prosperent_UserID']='400414';
		}
		if(!isset($n3u_configVars['self'])){
			$n3u_configVars['self']=str_replace(' ', '%20', ($n3u_ServerVars['PHP_SELF']));
		} // Auto determines current file path, ex: /index.php
		if(!isset($n3u_configVars['sid']) || $n3u_configVars['sid'] == NULL){
			$n3u_configVars['sid']=@$n3u_ServerVars['HTTP_HOST'];
		} // Auto determines Domain Name to track by domain, Shouldn't need to change
		if(!isset($n3u_configVars['SiteIndex']) || $n3u_configVars['SiteIndex'] == NULL){
			$n3u_configVars['SiteIndex']='index';
		} // Defaults to script name
		if(!isset($n3u_inputVars['x']) || $n3u_inputVars['x'] == NULL){
			$n3u_inputVars['x']=$n3u_configVars['SiteIndex'];
		}
		$n3u_inputVars['x']=strtolower($n3u_inputVars['x']); // Force lowercase
		if($n3u_inputVars['x'] == 'admin' && (!isset($n3u_inputVars['page']) || ($n3u_inputVars['page'] == NULL))){
			$n3u_inputVars['page']='dashboard';
		} // Sets default admin page
		if(!isset($n3u_configVars['SiteName']) || $n3u_configVars['SiteName'] == NULL){
			$n3u_configVars['SiteName']='n3u Niche Store';
		} // Defaults to script name
		if(!isset($n3u_configVars['SiteURL']) || $n3u_configVars['SiteURL'] == NULL){
			$n3u_configVars['SiteURL']='http://' . $n3u_ServerVars['HTTP_HOST'] . '/';
		} // Defaults to script name
		if(!isset($n3u_configVars['Supporter'])){
			$n3u_configVars['Supporter']=10;
		}elseif(isset($n3u_configVars['Supporter']) && !is_numeric($n3u_configVars['Supporter'])){
			$n3u_configVars['Supporter']='0';
		} // Defaults on and to 1 out of 10
		if(!isset($n3u_configVars['Template_Dir']) || $n3u_configVars['Template_Dir'] == NULL){
			$n3u_configVars['Template_Dir']='templates/';
		}
		if(!isset($n3u_configVars['Template_Name']) || $n3u_configVars['Template_Name'] == NULL){
			$n3u_configVars['Template_Name']='n3u';
		}
		if(!isset($n3u_configVars['userAgent']) || $n3u_configVars['userAgent'] == NULL){
			$n3u_configVars['userAgent']=@$n3u_ServerVars['HTTP_USER_AGENT'];
		} // Auto determines Visitors User Agent, Shouldn't need to change
		if(!isset($n3u_configVars['Version']) || $n3u_configVars['Version'] == NULL){
			$n3u_configVars['Version']='14.02.20';
		}
		if(!defined('admin') && $n3u_inputVars['x'] == 'item'){
			$n3u_configVars['api_key']=n3u_CheckApiKey($n3u_configVars['api_key'], $n3u_configVars['Supporter']);
			$n3u_configVars['img_size']='250x250';
		}
	}
	...
	function n3u_FetchBrands($val=NULL){
		...
		if(!$prosperentApi){
			die($n3u_lang['Prosperent_NoConnect']);
	...
	function n3u_GPL_Credits(){
		$GPL_Credits="\t\t" . 'n3u Niche Store - Custom Niche PHP Script' . PHP_EOL
				. "\t\t" . 'Copyright (C) 2012-2016 Strykerraven <https://github.com/Strykerraven/>' . PHP_EOL . PHP_EOL
	...
	function n3u_Pagination(){ // Pagination
		...
		if($n3u_totalItems > 1000){
			$n3u_totalItems=1000;
		} // force stop at 1000 results (current prosperent limit)
		...
		if($n3u_resultEnd > $n3u_totalItems){
			$n3u_resultEnd=$n3u_totalItems;
		} // if $n3u_resultEnd is more than $n3u_totalItems, show $n3u_resultEnd as $n3u_totalRecords
		if($n3u_inputVars['p'] > $n3u_totalPages){
		...
		if($n3u_configVars['CleanUrls'] == TRUE){
			...
			while(($i <= $n3u_totalPages) && ($n3u_totalPages > 1) && ($n3u_inputVars['p'] <= $n3u_totalPages)){ // while less than total pages AND total pages is greater than 1 AND  current page is equal to or less than total pages
				if($i == $n3u_inputVars['p']){
					echo "\t\t\t\t\t\t" . '<span class="current">' . $i . '</span>&nbsp;' . PHP_EOL;
				} // if current page display no link
			...
			if(($n3u_inputVars['p'] >= 1) && ($n3u_inputVars['p'] < $n3u_totalPages)){ // Display Next Link
				if($n3u_totalPages - $n3u_pageZone > $n3u_inputVars['p']){
					echo "\t\t\t\t\t\t\t" . ' ... ' . PHP_EOL;
				}
...
//			$txt_records = dns_get_record('n3u.com', DNS_TXT);
//			foreach($txt_records as $txt_record => $txt_values){
//				$txt_string = strstr($txt_values['txt'],'=', TRUE);
//				if($txt_string == 'nns-version'){
//					return(str_replace('nns-version=','',$txt_values['txt']));
//				}
//			}
//		}else{
//			$n3u_cacheFilePath = $n3u_configVars['cache_dir'] . 'version.txt';
//			if(file_exists($n3u_cacheFilePath) && time() - $n3u_configVars['lifetime'] < filemtime($n3u_cacheFilePath)){
...
//					}
//				}
//				$n3u_cacheFileDump = ob_get_contents();
//				ob_end_clean();
//				$n3u_cacheFileName = fopen($n3u_cacheFilePath, 'w');
//				fwrite($n3u_cacheFileName, $n3u_cacheFileDump);
//				fclose($n3u_cacheFileName);
...
//	}
	...
	function n3u_WriteConfig($domainConfig=NULL){
		...
		$configFile='<?php ' . PHP_EOL
				...
				. "\t" . '); // n3u Niche Store is brought to you by https://github.com/Strykerraven' . PHP_EOL
				. '?>';
		$fp=fopen($domainConfig, "w");
		fwrite($fp, $configFile);
		fclose($fp);
	...
	function n3u_WriteLanguage($n3u_lang_changes=NULL, $resetlang=FALSE){
		...
		if(isset($n3u_lang_changes) && $n3u_lang_changes != NULL){
			...
			$langFile .= "\t" . '); // n3u Niche Store is brought to you by https://github.com/Strykerraven' . PHP_EOL
					. '?>';
