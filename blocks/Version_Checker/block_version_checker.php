	...
	**/
	...
	switch($n3u_block_allowed){
		case "true": // If true, Block is allowed so we return data		
			$current_version= n3u_VersionChecker();
			...
			if($current_version == NULL){
				echo "\t\t\t\t\t" . '<span class="Invalid">' . $n3u_lang['Version_Invalid'] . '</span>' . PHP_EOL;
			}elseif($current_version > $n3u_configVars['Version']){
				echo "\t\t\t\t\t" . '<span class="Outdated">' . $n3u_lang['Outdated'] . '</span>' . PHP_EOL;
			}elseif($current_version < $n3u_configVars['Version']){
				echo "\t\t\t\t\t" . '<span class="Version_Tampered">' . $n3u_lang['Version_Tampered'] . $current_version . '</span>' . PHP_EOL;
			}else{
